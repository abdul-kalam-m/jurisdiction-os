  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <div className="flex flex-wrap gap-3">
        <select value={muniFilter} onChange={(e) => setMuniFilter(e.target.value)} className="rounded-md border border-slate-300 bg-white px-3 py-2 text-sm dark:border-slate-700 dark:bg-slate-900">
          <option value="">All municipalities</option>
          {municipalities.map((m) => <option key={m} value={m}>{m}</option>)}
        </select>
        <select value={actionFilter} onChange={(e) => setActionFilter(e.target.value)} className="rounded-md border border-slate-300 bg-white px-3 py-2 text-sm dark:border-slate-700 dark:bg-slate-900">
        ...
        {filtered.slice(0, 100).map((item, i) => (
          <li key={i} className="rounded-lg border border-slate-200 p-4 dark:border-slate-800">
            ...
            <div className="mt-2 flex flex-wrap items-center gap-2 text-xs text-slate-500 dark:text-slate-400">
              <span className="rounded bg-slate-100 px-2 py-0.5 dark:bg-slate-800">{item.use_type}</span>
              {item.variances_mentioned.map((v) => (
                <span key={v} className="rounded bg-slate-100 px-2 py-0.5 dark:bg-slate-800">{v}</span>
              ))}
      ...
      {filtered.length > 100 && (
        <p className="text-center text-sm text-slate-500">Showing first 100 of {filtered.length} matching items.</p>
      )}
    </div>
  )
}
